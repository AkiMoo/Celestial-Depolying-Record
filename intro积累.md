...
小型近地轨道卫星的巨型星座目前由各种私人和公共实体规划和部署。虽然全球连通性是主要的原理，但这些星座也有可能收集大量数据，例如用于地球观测。功率和带宽限制，加上隐私、限制延迟或弹性等动机，使得直接在星座内处理这些数据成为可能。我们考虑了由星座外参数服务器（PS）协调的机载联合学习（FL）的实现，并提出了一种新的通信方案来支持FL。它利用轨道内卫星间链路、卫星运动的可预测性和部分聚合来大幅减少训练时间和通信成本。特别是，对于一个有40颗卫星平均分布在五个近地轨道和PS分布在中地球轨道的星座，我们观察到训练过程时间加快了29倍，PS的流量比基线减少了8倍
